...
indices.spikes_in_A = zeros(length(datafiles),6000);
indices.spikes_in_B = zeros(length(datafiles),6000);
indices.spikes_not_in_A = zeros(length(datafiles),6000);
indices.spikes_not_in_B = zeros(length(datafiles),6000);
indices.index_A = zeros(length(datafiles),6000);
indices.index_B = zeros(length(datafiles),6000);
indices.theta_angle = zeros(length(datafiles),6000);
indices.beta_angle = zeros(length(datafiles),6000);
iterator = 0;
...
for file = datafiles'
    ...
    if rem (filenum,2)
        ...
        if indices.onset1(iterator) < 3600
            ...
            indices.theta_angle(iterator,:) = theta_angle1(low_time*10:(low_time*10)+5999);
            indices.beta_angle(iterator,:) = beta_angle1(low_time*10:(low_time*10)+5999);
        else
            iterator = iterator - 1;
        end
...
theta_angle_no_FS_2 = reshape(theta_angle_no_FS,1,length(range_FS)*6000);
theta_angle_FS_2 = reshape(theta_angle_FS,1,length(range_FS)*6000);
index_A_no_FS_2 = reshape(index_A_no_FS,1,length(range_FS)*6000);
index_A_FS_2 = reshape(index_A_FS,1,length(range_FS)*6000);

beta_angle_no_FS_2 = reshape(beta_angle_no_FS,1,length(range_FS)*6000);
beta_angle_FS_2 = reshape(beta_angle_FS,1,length(range_FS)*6000);
index_B_no_FS_2 = reshape(index_B_no_FS,1,length(range_FS)*6000);
index_B_FS_2 = reshape(index_B_FS,1,length(range_FS)*6000);

new_index_A_no_FS_2 = reshape(new_index_A_no_FS,1,length(range_FS)*6000);
new_index_A_FS_2 = reshape(new_index_A_FS,1,length(range_FS)*6000);
new_index_B_no_FS_2 = reshape(new_index_B_no_FS,1,length(range_FS)*6000);
new_index_B_FS_2 = reshape(new_index_B_FS,1,length(range_FS)*6000);

spikes_in_A_no_FS_2 = reshape(spikes_in_A_no_FS,1,length(range_FS)*6000);
spikes_in_A_FS_2 = reshape(spikes_in_A_FS,1,length(range_FS)*6000);
spikes_in_B_no_FS_2 = reshape(spikes_in_B_no_FS,1,length(range_FS)*6000);
spikes_in_B_FS_2 = reshape(spikes_in_B_FS,1,length(range_FS)*6000);
...
figure
foo1 = mean(reshape(s_theta_A_FS.Phase,6000,length(range_FS)));
foo2 = mean(abs(reshape(s_theta_A_FS.Spikes,6000,length(range_FS))));
plot(foo1,foo2);
hold on;
errorghost(abs(reshape(s_theta_A_FS.Spikes,6000,length(range_FS))),foo1,'b');

foo1 = mean(reshape(s_theta_A_no_FS.Phase,6000,length(range_FS)));
foo2 = mean(abs(reshape(s_theta_A_no_FS.Spikes,6000,length(range_FS))));
plot(foo1,foo2);
errorghost(abs(reshape(s_theta_A_no_FS.Spikes,6000,length(range_FS))),foo1,'r');
title('Theta dependence of assembly A');

figure
foo1 = mean(reshape(s_theta_B_FS.Phase,6000,length(range_FS)));
foo2 = mean(reshape(s_theta_B_FS.Spikes,6000,length(range_FS)));
plot(foo1,foo2);
hold on;
errorghost(reshape(s_theta_B_FS.Spikes,6000,length(range_FS)),foo1,'b');

foo1 = mean(reshape(s_theta_B_no_FS.Phase,6000,length(range_FS)));
foo2 = mean(reshape(s_theta_B_no_FS.Spikes,6000,length(range_FS)));
plot(foo1,foo2);
errorghost(reshape(s_theta_B_no_FS.Spikes,6000,length(range_FS)),foo1,'r');
title('Theta dependence of assembly B');

% figure
% foo1 = mean(reshape(s_beta_A_FS.Phase,6000,length(range_FS)));
% foo2 = mean(abs(reshape(s_beta_A_FS.Newindex,6000,length(range_FS))));
% plot(foo1,foo2);
% hold on;
% errorghost(abs(reshape(s_beta_A_FS.Newindex,6000,length(range_FS))),foo1,'b');
% 
% foo1 = mean(reshape(s_beta_A_no_FS.Phase,6000,length(range_FS)));
% foo2 = mean(abs(reshape(s_beta_A_no_FS.Newindex,6000,length(range_FS))));
% plot(foo1,foo2);
% errorghost(abs(reshape(s_beta_A_no_FS.Newindex,6000,length(range_FS))),foo1,'r');
