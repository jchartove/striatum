...
indices.delay = zeros(length(datafiles),1);
indices.spikes_in_q1 = zeros(length(datafiles),8000);
indices.spikes_in_q2 = zeros(length(datafiles),8000);
indices.spikes_in_q3 = zeros(length(datafiles),8000);
indices.spikes_in_q4 = zeros(length(datafiles),8000);
indices.theta_angle = zeros(length(datafiles),8000);
indices.beta_angle = zeros(length(datafiles),8000);
iterator = 0;
...
for file = datafiles'
    ...
    filenum = str2double(filenum{2});
    %if rem (filenum,2)
    iterator = iterator + 1
    indices.files(iterator) = filenum;
    load(file.name);
    
    mods = simulator_options.modifications;
    indices.onset1(iterator) = cell2mat(mods(11,3)');
    
    if indices.onset1(iterator) < 3400
        indices.onset2(iterator) = model.fixed_variables.D1_AMPAMSN_AMPA_onset2;
        indices.delay(iterator) = indices.onset2(iterator)-indices.onset1(iterator);
        
        low_time = indices.onset1(iterator)-200;
        time_index = (time >= low_time & time <= low_time+800);
        D1_spikes = diff(D1_V(time_index, :) >= 0) == 1;
        
        %all_spikes = movsum(sum(D1_spikes'),200);
        indices.spikes_in_q1(iterator,:) = movsum(sum(D1_spikes(:,1:25)'),200);
        indices.spikes_in_q2(iterator,:) = movsum(sum(D1_spikes(:,26:50)'),200);
        indices.spikes_in_q3(iterator,:) = movsum(sum(D1_spikes(:,51:75)'),200);
        indices.spikes_in_q4(iterator,:) = movsum(sum(D1_spikes(:,75:100)'),200);
        ...
        theta = wavelet_spectrogram(signal, 10000, 3, 3, 1);
        beta = wavelet_spectrogram(signal, 10000, 18, 6, 1);
        
        theta_angle1 = angle(theta);
        beta_angle1 = angle(beta);
        indices.theta_angle(iterator,:) = theta_angle1(low_time*10:(low_time*10)+7999);
        indices.beta_angle(iterator,:) = beta_angle1(low_time*10:(low_time*10)+7999);
    else
        iterator = iterator - 1;
    end
...
new_index_A = (spikes_in_A-spikes_in_B).*(spikes_in_A > spikes_in_B);
new_index_B = (spikes_in_B-spikes_in_A).*(spikes_in_B > spikes_in_A);
%%
...
theta_angle_no_FS_2 = reshape(theta_angle_no_FS,1,length(range_FS)*8000);
theta_angle_FS_2 = reshape(theta_angle_FS,1,length(range_FS)*8000);
index_A_no_FS_2 = reshape(index_A_no_FS,1,length(range_FS)*8000);
index_A_FS_2 = reshape(index_A_FS,1,length(range_FS)*8000);

beta_angle_no_FS_2 = reshape(beta_angle_no_FS,1,length(range_FS)*8000);
beta_angle_FS_2 = reshape(beta_angle_FS,1,length(range_FS)*8000);
index_B_no_FS_2 = reshape(index_B_no_FS,1,length(range_FS)*8000);
index_B_FS_2 = reshape(index_B_FS,1,length(range_FS)*8000);

new_index_A_no_FS_2 = reshape(new_index_A_no_FS,1,length(range_FS)*8000);
new_index_A_FS_2 = reshape(new_index_A_FS,1,length(range_FS)*8000);
new_index_B_no_FS_2 = reshape(new_index_B_no_FS,1,length(range_FS)*8000);
new_index_B_FS_2 = reshape(new_index_B_FS,1,length(range_FS)*8000);

spikes_in_A_no_FS_2 = reshape(spikes_in_A_no_FS,1,length(range_FS)*8000);
spikes_in_A_FS_2 = reshape(spikes_in_A_FS,1,length(range_FS)*8000);
spikes_in_B_no_FS_2 = reshape(spikes_in_B_no_FS,1,length(range_FS)*8000);
spikes_in_B_FS_2 = reshape(spikes_in_B_FS,1,length(range_FS)*8000);
...
figure
foo1 = mean(reshape(s_theta_A_FS.Phase,8000,length(range_FS)));
foo2 = mean(abs(reshape(s_theta_A_FS.Spikes,8000,length(range_FS))));
plot(foo1,foo2);
hold on;
errorghost(abs(reshape(s_theta_A_FS.Spikes,8000,length(range_FS))),foo1,'b');

foo1 = mean(reshape(s_theta_A_no_FS.Phase,8000,length(range_FS)));
foo2 = mean(abs(reshape(s_theta_A_no_FS.Spikes,8000,length(range_FS))));
plot(foo1,foo2);
errorghost(abs(reshape(s_theta_A_no_FS.Spikes,8000,length(range_FS))),foo1,'r');
title('Theta dependence of assembly A');

figure
foo1 = mean(reshape(s_theta_B_FS.Phase,8000,length(range_FS)));
foo2 = mean(reshape(s_theta_B_FS.Spikes,8000,length(range_FS)));
plot(foo1,foo2);
hold on;
errorghost(reshape(s_theta_B_FS.Spikes,8000,length(range_FS)),foo1,'b');

foo1 = mean(reshape(s_theta_B_no_FS.Phase,8000,length(range_FS)));
foo2 = mean(reshape(s_theta_B_no_FS.Spikes,8000,length(range_FS)));
plot(foo1,foo2);
errorghost(reshape(s_theta_B_no_FS.Spikes,8000,length(range_FS)),foo1,'r');
title('Theta dependence of assembly B');

% figure
% foo1 = mean(reshape(s_beta_A_FS.Phase,8000,length(range_FS)));
% foo2 = mean(abs(reshape(s_beta_A_FS.Newindex,8000,length(range_FS))));
% plot(foo1,foo2);
% hold on;
% errorghost(abs(reshape(s_beta_A_FS.Newindex,8000,length(range_FS))),foo1,'b');
%
% foo1 = mean(reshape(s_beta_A_no_FS.Phase,8000,length(range_FS)));
% foo2 = mean(abs(reshape(s_beta_A_no_FS.Newindex,8000,length(range_FS))));
% plot(foo1,foo2);
% errorghost(abs(reshape(s_beta_A_no_FS.Newindex,8000,length(range_FS))),foo1,'r');
