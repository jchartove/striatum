function generate_img_wilson(directory,decim,datatype_range)
	cd(directory);
	...
	for file = datafiles'
        ...
        if decim
            ...
            soma_soma_wilson_K_a = zeros(new_T,numcells);
            soma_soma_wilson_K_b = zeros(new_T,numcells);
            soma_soma_wilson_Kdr_n = zeros(new_T,numcells);
            soma_soma_wilson_Na_h = zeros(new_T,numcells);
            soma_soma_soma_soma_iSYN_s = zeros(new_T,numcells);
			
            dend_v = zeros(new_T,numcells);
            dend_dend_wilson_K_a = zeros(new_T,numcells);
            dend_dend_wilson_K_b = zeros(new_T,numcells);
            dend_dend_wilson_Kdr_n = zeros(new_T,numcells);
            dend_dend_wilson_Na_h = zeros(new_T,numcells);
            ...
            for m = 1:numcells
                ...
                soma_soma_wilson_K_a(:,m) = decimate(sim_data.soma_soma_wilson_K_a(:,m),10); 
                soma_soma_wilson_K_b(:,m) = decimate(sim_data.soma_soma_wilson_K_b(:,m),10);
                soma_soma_wilson_Kdr_n(:,m) = decimate(sim_data.soma_soma_wilson_Kdr_n(:,m),10);
                soma_soma_wilson_Na_h(:,m) = decimate(sim_data.soma_soma_wilson_Na_h(:,m),10);
                soma_soma_soma_soma_iSYN_s(:,m) = decimate(sim_data.soma_soma_soma_soma_iSYN_s(:,m),10);
				
                dend_v(:,m) = decimate(sim_data.dend_v(:,m),10);
                dend_dend_wilson_K_a(:,m) = decimate(sim_data.dend_dend_wilson_K_a(:,m),10);
                dend_dend_wilson_K_b(:,m) = decimate(sim_data.dend_dend_wilson_K_b(:,m),10);
                dend_dend_wilson_Kdr_n(:,m) = decimate(sim_data.dend_dend_wilson_Kdr_n(:,m),10);
                dend_dend_wilson_Na_h(:,m) = decimate(sim_data.dend_dend_wilson_Na_h(:,m),10);
			end
        ...
        handle4 = figure;
		if datatype == 1
			plot(time(T_start+1:end),spike_indicator(1,:),time(T_start+1:end),soma_soma_wilson_Na_h(T_start+1:end,1), ... 
            time(T_start+1:end),soma_soma_wilson_Kdr_n(T_start+1:end,1), time(T_start+1:end),soma_soma_wilson_K_a(T_start+1:end,1), ...
            time(T_start+1:end),soma_soma_wilson_K_b(T_start+1:end,1), time(T_start+1:end), soma_soma_soma_soma_iSYN_s(T_start+1:end,1));
