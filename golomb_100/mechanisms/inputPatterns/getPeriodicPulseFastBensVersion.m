function s4 = getPeriodicPulseFastBensVersion(freq,width,shift,T,dt,onset,offset,Npop,kernel_type,width2_rise,center_flag,norm_in,plot_demo_on)
...
if strcmp(norm, 'total')
    s4 = sum(t>=onset & t<=offset)*dt*s3/sum(s3*dt); % This prefactor is just so this code works with a "conductance" of 0-.5.
    % disp('Norm: total.')
else
    s4 = s3;
end
