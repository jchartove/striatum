function [Y,T] = odefun_20150424_165417
tspan=[0 200]; dt=0.01;
...
soma_soma_iSYN_netcon = rand((2),(2))<(0.3);
soma_soma_iSYN_netcon = soma_soma_iSYN_netcon.*(1-eye((2)));
dendrite_soma_iCOM_compcon = eye((2),(2));
...
soma_soma_iSYN_ISYN = @(V,soma_soma_iSYN_s) (((0.3)/((2)*(0.3))).*(soma_soma_iSYN_netcon*soma_soma_iSYN_s).*(V-(-85)));
...
for k=2:nstep
  t=T(k-1);
  F=((-((36)*(soma_K_n(:,k-1).^4).*(soma_v(:,k-1)-(-77))))+((-((120)*soma_Na_h(:,k-1).*(soma_Na_m(:,k-1).^3).*(soma_v(:,k-1)-(50))))+((-((0.3)*(soma_v(:,k-1)-(-54.4))))+((-((((0.3)/((2)*(0.3))).*(soma_soma_iSYN_netcon*soma_soma_iSYN_s(:,k-1)).*(soma_v(:,k-1)-(-85)))))+((((0.15).*sum((((dendrite_v(:,k-1)*ones(1,size(dendrite_v(:,k-1),1)))'-(soma_v(:,k-1)*ones(1,size(soma_v(:,k-1),1))))).*dendrite_soma_iCOM_compcon,2)))+0)))))/(1);
