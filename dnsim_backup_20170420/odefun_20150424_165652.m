function [Y,T] = odefun_20150424_165652
tspan=[0 200]; dt=0.01;
T=tspan(1):dt:tspan(2); nstep=length(T);
fileID = 1; nreports = 5; enableLog = 1:(nstep-1)/nreports:nstep;enableLog(1) = [];
fprintf('\nSimulation interval: %g-%g\n',tspan);fprintf('Starting integration (euler, dt=%g)\n',dt);dendrite_soma_iCOM_compcon = eye((2),(2));
dendrite_iMultiPoissonExp_Ge = multi_Poisson((2),(127),(2),(10),(1),2,.5,(2000),(0.01));
dendrite_iMultiPoissonExp_Gi = multi_Poisson((2),(73),(2),(10),(1),5,.5,(2000),(0.01));
soma_dendrite_iCOM_compcon = eye((2),(2));
dendrite_dendrite_iGAP_UB = max((2),(2));
dendrite_dendrite_iGAP_Xpre = linspace(1,dendrite_dendrite_iGAP_UB,(2))'*ones(1,(2));
dendrite_dendrite_iGAP_Xpost = (linspace(1,dendrite_dendrite_iGAP_UB,(2))'*ones(1,(2)))';
dendrite_dendrite_iGAP_mask = rand((2),(2))<(0.3);
dendrite_dendrite_iGAP_mask = dendrite_dendrite_iGAP_mask.*(1-eye((2)));
...
dendrite_dendrite_iGAP_IGAP = @(V1,V2) ((0)/((0.3)*(2))).*sum(((V1*ones(1,size(V1,1)))'-(V2*ones(1,size(V2,1)))).*dendrite_dendrite_iGAP_mask,2);
soma_v = zeros(2,nstep);
soma_v(:,1) = [-65 -65];
soma_K_n = zeros(2,nstep);
soma_K_n(:,1) = [0.317       0.317];
soma_Na_m = zeros(2,nstep);
soma_Na_m(:,1) = [0.052       0.052];
soma_Na_h = zeros(2,nstep);
soma_Na_h(:,1) = [0.596       0.596];
dendrite_v = zeros(2,nstep);
dendrite_v(:,1) = [-65 -65];
dendrite_K_n = zeros(2,nstep);
dendrite_K_n(:,1) = [0.317       0.317];
dendrite_Na_m = zeros(2,nstep);
dendrite_Na_m(:,1) = [0.052       0.052];
dendrite_Na_h = zeros(2,nstep);
dendrite_Na_h(:,1) = [0.596       0.596];
tstart = tic;
for k=2:nstep
  t=T(k-1);
  F=((-((36)*(soma_K_n(:,k-1).^4).*(soma_v(:,k-1)-(-77))))+((-((120)*soma_Na_h(:,k-1).*(soma_Na_m(:,k-1).^3).*(soma_v(:,k-1)-(50))))+((-((0.3)*(soma_v(:,k-1)-(-54.4))))+((((0.15).*sum((((dendrite_v(:,k-1)*ones(1,size(dendrite_v(:,k-1),1)))'-(soma_v(:,k-1)*ones(1,size(soma_v(:,k-1),1))))).*dendrite_soma_iCOM_compcon,2)))+0))))/(1);
  ...
  F=((-((3.6)*(dendrite_K_n(:,k-1).^4).*(dendrite_v(:,k-1)-(-77))))+((-((12)*dendrite_Na_h(:,k-1).*(dendrite_Na_m(:,k-1).^3).*(dendrite_v(:,k-1)-(50))))+(((10))+((-((0.03)*(dendrite_v(:,k-1)-(-54.4))))+((-((((0.1).*dendrite_iMultiPoissonExp_Ge(:,max(1,round(dendrite_v(:,k-1)/(0.01))))).*(dendrite_v(:,k-1) - (0))) + (((0.1).*dendrite_iMultiPoissonExp_Gi(:,max(1,round(dendrite_v(:,k-1)/(0.01))))).*(dendrite_v(:,k-1) - (-85)))))+((((0.15).*sum((((soma_v(:,k-1)*ones(1,size(soma_v(:,k-1),1)))'-(dendrite_v(:,k-1)*ones(1,size(dendrite_v(:,k-1),1))))).*soma_dendrite_iCOM_compcon,2)))+(((((0)/((0.3)*(2))).*sum(((dendrite_v(:,k-1)*ones(1,size(dendrite_v(:,k-1),1)))'-(dendrite_v(:,k-1)*ones(1,size(dendrite_v(:,k-1),1)))).*dendrite_dendrite_iGAP_mask,2)))+0)))))))/(1);
...
Y=cat(1,soma_v,soma_K_n,soma_Na_m,soma_Na_h,dendrite_v,dendrite_K_n,dendrite_Na_m,dendrite_Na_h)';
