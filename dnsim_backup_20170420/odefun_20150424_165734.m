function [Y,T] = odefun_20150424_165734
tspan=[0 200]; dt=0.01;
...
for k=2:nstep
  ...
  F=((-((3.6)*(dendrite_K_n(:,k-1).^4).*(dendrite_v(:,k-1)-(-77))))+((-((12)*dendrite_Na_h(:,k-1).*(dendrite_Na_m(:,k-1).^3).*(dendrite_v(:,k-1)-(50))))+(((10))+((-((0.03)*(dendrite_v(:,k-1)-(-54.4))))+((-((((0.1).*dendrite_iMultiPoissonExp_Ge(:,max(1,round(dendrite_v(:,k-1)/(0.01))))).*(dendrite_v(:,k-1) - (0))) + (((0.1).*dendrite_iMultiPoissonExp_Gi(:,max(1,round(dendrite_v(:,k-1)/(0.01))))).*(dendrite_v(:,k-1) - (-85)))))+((((0.15).*sum((((soma_v(:,k-1)*ones(1,size(soma_v(:,k-1),1)))'-(dendrite_v(:,k-1)*ones(1,size(dendrite_v(:,k-1),1))))).*soma_dendrite_iCOM_compcon,2)))+0))))))/(1);
