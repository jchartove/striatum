function [Y,T] = odefun_20151007_181241665_job0001
tspan=[0 4000]; dt=0.01;
T=tspan(1):dt:tspan(2); nstep=length(T);
nreports = 5; tmp = 1:(nstep-1)/nreports:nstep; enableLog = tmp(2:end);
fprintf('\nThe odefun file used is: odefun_20151007_181241665_job0001 \n');
fprintf('\nSimulation interval: %g-%g\n',tspan(1),tspan(2));
fprintf('Starting integration (rk4, dt=%g)\n',dt);
soma_soma_soma_soma_iSYN_width = inf;
soma_soma_soma_soma_iSYN_Nmax = max((2),(2));
soma_soma_soma_soma_iSYN_srcpos = linspace(1,soma_soma_soma_soma_iSYN_Nmax,(2))'*ones(1,(2));
soma_soma_soma_soma_iSYN_dstpos = (linspace(1,soma_soma_soma_soma_iSYN_Nmax,(2))'*ones(1,(2)))';
soma_soma_soma_soma_iSYN_netcon = rand((2),(2))<(0);
dend_soma_dend_soma_iCOM_compcon = eye((2),(2));
dend_dend_golomb_K_gd_dend = (8)/10;
dend_dend_golomb_Kdr_gkdr_dend = (225)/10;
dend_dend_golomb_Na_gna_dend = (112)/10;
dend_dend_iMultiPoissonExp_Ge = corr_Poisson((2), (50), (2), (10), (1), 2, .5, (2000), (0.01), (0));
dend_dend_iMultiPoissonExp_Gi = corr_Poisson((2), (0), (2), (10), (1), 5, .5, (2000), (0.01), (0));
soma_dend_soma_dend_iCOM_compcon = eye((2),(2));
dend_dend_dend_dend_iGAP_Nmax = max((2),(2));
dend_dend_dend_dend_iGAP_Xpre = linspace(1,dend_dend_dend_dend_iGAP_Nmax,(2))'*ones(1,(2));
dend_dend_dend_dend_iGAP_Xpost = (linspace(1,dend_dend_dend_dend_iGAP_Nmax,(2))'*ones(1,(2)))';
dend_dend_dend_dend_iGAP_mask = rand((2),(2))<(1);
dend_dend_dend_dend_iGAP_mask = triu(dend_dend_dend_dend_iGAP_mask);
dend_dend_dend_dend_iGAP_mask = dend_dend_dend_dend_iGAP_mask + dend_dend_dend_dend_iGAP_mask.';
dend_dend_dend_dend_iGAP_mask = dend_dend_dend_dend_iGAP_mask - diag(diag(dend_dend_dend_dend_iGAP_mask));
soma_v = zeros(2,nstep);
soma_v(:,1) = [0  0];
soma_soma_golomb_K_a = zeros(2,nstep);
soma_soma_golomb_K_a(:,1) = [0.39461     0.39461];
soma_soma_golomb_K_b = zeros(2,nstep);
soma_soma_golomb_K_b(:,1) = [0.57384     0.57384];
soma_soma_golomb_Kdr_n = zeros(2,nstep);
soma_soma_golomb_Kdr_n(:,1) = [0.38822     0.38822];
soma_soma_golomb_Na_h = zeros(2,nstep);
soma_soma_golomb_Na_h(:,1) = [0.53956     0.53956];
soma_soma_soma_soma_iSYN_s = zeros(2,nstep);
soma_soma_soma_soma_iSYN_s(:,1) = [0.1         0.1];
dend_v = zeros(2,nstep);
dend_v(:,1) = [0  0];
dend_dend_golomb_K_a = zeros(2,nstep);
dend_dend_golomb_K_a(:,1) = [0.25996     0.25996];
dend_dend_golomb_K_b = zeros(2,nstep);
dend_dend_golomb_K_b(:,1) = [0.20549     0.20549];
dend_dend_golomb_Kdr_n = zeros(2,nstep);
dend_dend_golomb_Kdr_n(:,1) = [0.2665      0.2665];
dend_dend_golomb_Na_h = zeros(2,nstep);
dend_dend_golomb_Na_h(:,1) = [0.91008     0.91008];
tstart = tic;
for k=2:nstep
  t=T(k-1);
  soma_v1=((-((8).*(soma_soma_golomb_K_a(:,k-1).^3).*soma_soma_golomb_K_b(:,k-1).*(soma_v(:,k-1)-(-90))))+((-((225).*(soma_soma_golomb_Kdr_n(:,k-1).^(2)).*(soma_v(:,k-1)-(-90))))+((-((112).*soma_soma_golomb_Na_h(:,k-1).*((1./(1+exp(-(soma_v(:,k-1)-(-24))./11.5))).^3).*(soma_v(:,k-1)-(50))))+((-((0.03)*(soma_v(:,k-1)-(-54.4))))+((-(((0.005).*(soma_soma_soma_soma_iSYN_netcon*soma_soma_soma_soma_iSYN_s(:,k-1)).*(soma_v(:,k-1)-(-85)))))+((((0.15).*sum((((dend_v(:,k-1)*ones(1,size(dend_v(:,k-1),1)))'-(soma_v(:,k-1)*ones(1,size(soma_v(:,k-1),1))))).*dend_soma_dend_soma_iCOM_compcon,2)))+0))))));
  ...
  t=t+0.5*dt;
  soma_v4=((-((8).*((soma_soma_golomb_K_a(:,k-1)+dt*soma_soma_golomb_K_a3).^3).*(soma_soma_golomb_K_b(:,k-1)+dt*soma_soma_golomb_K_b3).*((soma_v(:,k-1)+dt*soma_v3)-(-90))))+((-((225).*((soma_soma_golomb_Kdr_n(:,k-1)+dt*soma_soma_golomb_Kdr_n3).^(2)).*((soma_v(:,k-1)+dt*soma_v3)-(-90))))+((-((112).*(soma_soma_golomb_Na_h(:,k-1)+dt*soma_soma_golomb_Na_h3).*((1./(1+exp(-((soma_v(:,k-1)+dt*soma_v3)-(-24))./11.5))).^3).*((soma_v(:,k-1)+dt*soma_v3)-(50))))+((-((0.03)*((soma_v(:,k-1)+dt*soma_v3)-(-54.4))))+((-(((0.005).*(soma_soma_soma_soma_iSYN_netcon*(soma_soma_soma_soma_iSYN_s(:,k-1)+dt*soma_soma_soma_soma_iSYN_s3)).*((soma_v(:,k-1)+dt*soma_v3)-(-85)))))+((((0.15).*sum(((((dend_v(:,k-1)+dt*dend_v3)*ones(1,size((dend_v(:,k-1)+dt*dend_v3),1)))'-((soma_v(:,k-1)+dt*soma_v3)*ones(1,size((soma_v(:,k-1)+dt*soma_v3),1))))).*dend_soma_dend_soma_iCOM_compcon,2)))+0))))));
